...
const CONVERTER_DEFS = Object.freeze({
  ...
  stagedjson_to_inputxml: {
    ...
    fields: [
      ...
      { key: 'temperature1', label: 'Temperature 1 (Â°C)', type: 'number', step: '0.1' },
...
function _createWorkerRuntime() {
  const worker = new Worker(new URL('../converters/py-worker.js?v=20260515-cii-compat-check', import.meta.url), { type: 'module' });
  ...
  runBtnEl.addEventListener('click', async () => {
    ...
    try {
      ...
      if (selectedConverter === 'pcf_continuity_check') {
        ...
        const pcfText = _decodeTextUtf8(primaryBytes);
        const { analyzePcfTextContinuity } = await import('../rvm-pcf-extract/RvmPcfContinuityChecker.js');
        const report = analyzePcfTextContinuity(pcfText, runValues);
        ...
        const outputName = `${_baseNameWithoutExtension(primaryFile.name)}_pcf_continuity_report.json`;
        ...
        setStatus(
          `Continuity check complete: ${report.fatalCount || 0} fatal mismatch(es), ${report.fixableCount || 0} fixable gap(s)` +
        ...
        outputEl.innerHTML = `
          <div class="model-converters-output-row">
            <strong>${_esc(output.name)}</strong>
            <button type="button" class="model-converters-download-btn" data-output-index="0">Download</button>
          </div>
        `;
        outputEl.querySelector('[data-output-index="0"]')?.addEventListener('click', () => _downloadOutput(output));
        const logLines = [
        ...
        setLogs(logLines);
        _3DModelConv_resetPreview('PCF continuity report generated.');
