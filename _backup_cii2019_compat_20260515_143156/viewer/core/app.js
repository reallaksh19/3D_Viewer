...
import { renderModelConvertersTab } from '../tabs/model-converters-tab.js?v=20260515-cii-compat-check';
...
// Map of tabId -> { panel: HTMLElement, destroyFn: Function|null }
...
function _switchTab(tabId) {
  ...
  _tabPanels.forEach(({ panel }) => {
    panel.style.display = 'none';
  });

  // If panel already exists for this tab, just show it
  if (_tabPanels.has(tabId)) {
    _tabPanels.get(tabId).panel.style.display = '';
    return;
  }

  // First time mounting this tab — create a dedicated panel div
  ...
  let destroyFn = null;
  if (tabDef.render) {
    const result = tabDef.render(panel);
    if (typeof result === 'function') destroyFn = result;
  }

  _tabPanels.set(tabId, { panel, destroyFn });
}
