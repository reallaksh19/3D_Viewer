...
def _validate_miscel_1(
    ...
    if hanger_count > 0:
        ...
        for text_index in range(hanger_count * 2):
            line_no, line_text = rows[cursor]
            tokens = _tokens(line_text)
            if not tokens or not _is_int(tokens[0]):
                _add_error(errors, "miscel_hanger_text_length", "MISCEL_1", line_no, f"Hanger TAG/GUID row {text_index + 1} must start with an integer length token.")
            cursor += 1
