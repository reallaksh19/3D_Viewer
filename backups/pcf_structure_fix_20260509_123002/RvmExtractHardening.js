...
export class RvmExtractHardening {
  ...
  buildPcfAuditReport(rows = [], pcfTextByPipelineRef = {}, sourceLabel = '') {
    ...
    for (const row of rows) {
      ...
      if (
        ...
        push(
          'ERROR',
          'PCF-MISSING-COORDINATE',
          'SUPPORT row missing supportCoor/cp/ep1',
          row,
      ...
      if (type === 'PIPE' && _clean(row.skey)) {
        ...
        push(
          'WARNING',
          'PCF-PIPE-SKEY-RISK',
          'PIPE row has SKEY; suppress by default in emitter phase.',
          row,
          { skey: row.skey }
        );
      }
    ...
    for (const [ref, text] of Object.entries(pcfTextByPipelineRef || {})) {
      ...
      const pipeBlocks = body
        .split(/\n(?=[A-Z][A-Z0-9-]*\n)/g)
        .filter(block => /^PIPE\s*$/m.test(block));
      ...
      if (pipeSkeyBlocks) {
        push(
          'WARNING',
          'PCF-TEXT-PIPE-SKEY',
          `Generated PCF ${ref} has PIPE SKEY block(s).`,
          { pipelineRef: ref }
        );
      }
      ...
      const originLines =
        body.match(
          /^\s*(END-POINT|CENTRE-POINT|BRANCH1-POINT|CO-ORDS)\s+0\.000\s+0\.000\s+0\.000\s*$/gm
        ) || [];
  ...
  downloadAllPcf(pcfTextByPipelineRef) {
    ...
    if (entries.length === 1) {
      ...
      _downloadBlob(
        entry.name,
        new Blob([entry.content], { type: 'text/plain;charset=utf-8' })
      );
