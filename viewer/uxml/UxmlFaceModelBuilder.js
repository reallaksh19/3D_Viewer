...
function addRoleFace(out, uxml, component, role, faceOptions = {}) {
  const anchor = anchorByRole(uxml, component.id, role);
  const port = anchor
...
function buildPipeLikeFaces(uxml, component, record, add) {
  ...
  const ep1 = ep1Face?.point || null;
  const ep2 = ep2Face?.point || null;

  const axis = buildAxisVector(ep1, ep2);
...
function makeSummary(out) {
  ...
  const branchVectorCount = out.components.filter(component => !!component.branchVector).length;
  const highConfidenceBranchVectorCount = out.components.filter(
    component => component.branchVectorConfidence === 'HIGH'
  ).length;
  const mediumConfidenceBranchVectorCount = out.components.filter(
  ...
  return {
    ...
    branchVectorCount,
    highConfidenceBranchVectorCount,
    mediumConfidenceBranchVectorCount,
