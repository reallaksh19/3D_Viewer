...
// T12: downloadAllPcf — Node env (no document), returns filenames
{
  const result = h.downloadAllPcf({ 'LINE-A': 'pcf text A', 'LINE-B': 'pcf text B' });
  assert(Array.isArray(result) && result.includes('LINE-A.pcf') && result.includes('LINE-B.pcf'),
    'T12: downloadAllPcf returns filenames in Node env');
}
