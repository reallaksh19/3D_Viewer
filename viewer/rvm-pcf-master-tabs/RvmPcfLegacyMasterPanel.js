      ...
      <div class="rvm-master-field-grid">
        ${Object.keys(def.defaultMap || {}).map(field => `
          <label class="rvm-master-field">
            ...
            <select data-field-map="${esc(field)}" style="min-width:180px;">
              <option value="">-- Not mapped --</option>
              ${headers.map(h => {
                const sel = fieldMap[field] === h ? 'selected' : '';
                // Show preview text in options, but show just column name when selected
                const preview = esc(colPreview[h] || h);
                const label = sel ? esc(h) : preview;
                return `<option value="${esc(h)}" ${sel} data-preview="${preview}">${label}</option>`;
              }).join('')}
            </select>
          </label>
        `).join('')}
  ...
  return `
    <div class="rvm-master-card">
      ...
      <div class="rvm-master-field-map" style="margin-top:1rem;background:#1e2025;padding:12px;border:1px solid #333;border-radius:6px;">
        ...
        <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap;">
          <input type="text" id="cfg-link-url-${esc(masterKey)}" value="${esc(localStorage.getItem(`rvm_pcf_link_${masterKey}`) || '')}" style="padding:4px;background:#111;color:#fff;border:1px solid #444;border-radius:4px;flex:1;" placeholder="URL (e.g. /Docs/Masters/PCF_MAT_MAP.json)">
      ...
      <div class="rvm-master-split">
        ...
        <section>
          ...
          ${masterKey === 'supportMapping' ? renderEditableSupportMappingTable(rows) : renderRowsTable(rows, 200)}
        </section>
      </div>
    </div>
  `;
...
export function mountRvmPcfLegacyMasterPanel(container) {
  ...
  (() => {
    ...
    setTimeout(async () => {
      ...
      const currentMasters = state.rvmPcfExtract?.masters || {};
      for (const k of keys) {
        const url = localStorage.getItem(`rvm_pcf_link_${k}`);
        if (url) {
           try {
             ...
             if (res.ok) {
               ...
               if (Array.isArray(data)) {
                 currentMasters[k] = { rows: data, fieldMap: autoMapFields(headersFromRows(data), MASTER_DEFS[k], data) };
                 changed = true;
               }
             }
           } catch (e) {
        ...
        if (urlInput) {
          try {
             ...
             if (res.ok) {
               ...
               if (Array.isArray(data)) {
                 const currentMasters = state.rvmPcfExtract?.masters || {};
                 currentMasters[key] = { rows: data, fieldMap: autoMapFields(headersFromRows(data), MASTER_DEFS[key], data) };
                 updateRvmPcfExtractState({ masters: currentMasters }, 'master-link-load');
                 saveMastersToLocalStorage(currentMasters);
                 draw();
