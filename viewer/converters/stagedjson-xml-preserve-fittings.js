...
function installPanel() {
  ...
  const root = document.querySelector('#tab-content'); if (!root) return;
  if (!/Model Converter|StagedJSON\s*->\s*XML/i.test(root.textContent || '')) return;
  const leftRail = root.querySelector('.model-converters-left');
  if (!leftRail) return;

  let panel = root.querySelector('[data-sjxml-preserve-panel]');
  if (panel && panel.parentElement !== leftRail) {
    leftRail.appendChild(panel);
  }
  if (!panel) {
    panel = document.createElement('section');
    panel.className = 'model-converters-card';
    panel.dataset.sjxmlPreservePanel = '1';
    panel.style.marginTop = '12px';
    panel.innerHTML = '<h3>StagedJSON -> XML (preserve fittings)</h3><p class="muted">Preserves VALV, FLAN, GASK, ELBO/BEND, OLET, TEE and ATTA using the existing PSI116 Node XML structure.</p><label class="model-converters-file"><span>Staged JSON</span><input type="file" accept=".json,.JSON" data-sjxml-file><small>Choose staged hierarchy JSON file.</small></label><div class="model-converters-actions"><button class="btn-primary" type="button" data-sjxml-run>Convert preserving fittings</button></div><pre data-sjxml-status style="white-space:pre-wrap;max-height:160px;overflow:auto;"></pre>';
    leftRail.appendChild(panel);
  }
  const status = panel.querySelector('[data-sjxml-status]');
