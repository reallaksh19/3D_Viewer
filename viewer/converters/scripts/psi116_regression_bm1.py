...
5. Run xml_to_cii.py unchanged.
6. Parse the xml_to_cii.py summary and fail if source has special components but CII writes all special counts as zero.
"""
...
def parse_summary(stdout: str) -> dict:
    ...
    if not match:
        raise SystemExit('Could not parse xml_to_cii.py summary line.')
...
def main() -> int:
    ...
    cii_run = run_cmd([sys.executable, str(SCRIPT_DIR / 'xml_to_cii.py'), '--input', str(xml_path), '--output', str(cii_path)], REPO_ROOT)
