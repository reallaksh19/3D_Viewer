...
- Use BM_CII_DEFAULTVALUES.CII only to complete missing/sentinel hanger values.
    ...
    for elem in root.iter():
        ...
        hangers.append(
            XmlHanger(
                ...
                num_hgr=_to_int(elem.attrib.get("NUM_HGR"), 1),
                hgr_table=hgr_table,
...
def extract_hanger_default_profile(default_profile_cii: Path) -> HangerDefaultProfile:
    """
    Extracts default hanger block shape from BM_CII_DEFAULTVALUES.CII when available.
    ...
    fallback = HangerDefaultProfile(
        ...
        default_line_1=[1, 25.0, 0.0, 9999.99, 1.0, 0.0],
        ...
        hgrdat_template=[0.0, 25.0, 0.0, 9999.99, 0.0, 0.0, 0.0, 9999.99, 0.0, 0.0, 0.0],
        # anchor1, anchor2, dof1, dof2
        ihgrfree_template=[0, 0, 0, 0],
        ihgrnum_template=1,
        ihgrtable_template=1,
        ihgrshort_template=1,
        ihgrcn_template=0,
        execution_lines=[
            _format_i([0, 0, 0, 0]) + f"{0.0:13.6G}{0:13d}",
            _format_i([0]) + f"{0.0:13.6G}{70.0:13.6G}" + "".join(f"{0:13d}" for _ in range(3)),
            _format_i([0, 0, 0, 0]) + f"{0.0:13.6G}{0:13d}",
            _format_i([0]),
        ],
    )

    if not default_profile_cii.exists():
        return fallback
